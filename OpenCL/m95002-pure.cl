...
KERNEL_FQ void m95002_init (KERN_ATTR_TMPS (murmur64a_tmp_t))
{
  /**
   * modifier
   */
  ...
  if (password_length < 9 || !(password_length % 2) || password_length > 71)
  {
    //printf("WARNING: password must be of odd length >= 9 and <= 71\n");
    tmps[gid].prefix_hash = 0x0;
    return;
  }

  u32 salt_idx = ((password_length - 1) / 2) - 4;
...
KERNEL_FQ void m95002_loop (KERN_ATTR_TMPS (murmur64a_tmp_t))
{

}

KERNEL_FQ void m95002_comp (KERN_ATTR_TMPS (murmur64a_tmp_t))
{
  const u64 lid = get_local_id (0);
  const u64 gid = get_global_id (0);
